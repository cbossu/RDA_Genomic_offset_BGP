...
```{r}
env<-read_delim("input/cawaEnv_wc2.1.txt",delim="\t")
long_lat<-env %>% dplyr::select(long,lat)
cawaSampled <- read.xlsx("input/cawa_seq_metadata.xlsx", sheetIndex=1) %>% rename(long=X.long,lat=X.lat)
...
### Added individual column in awk using paste function

```{r}
gen.impall<-read.delim(file="input/cawa.bqsr.miss0.2.qual30.1X.fst85.filtRda.noIndel.gl_impute.lfmm_indcol",sep="\t",row.names=1,header=F)
